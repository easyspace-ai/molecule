...
- Plugins must not import `@easyspace/kernel` internals — only `@easyspace/plugin-api` facades.
...
3. `loadExtensionPlugins()` in `@easyspace/plugin-runtime` fetches manifests and dynamic-imports modules at startup.
